...
NAEEM_test_SHA1() {
  ...
  NAEEM_result result = NAEEM_crypto_SHA1(message, strlen((const char*)message), hash);
  ...
  NAEEM_byte expected_hash_value[20] = {0x93, 0x62, 0x5d, 0xcb, 0x47, 0x9d, 0x1c, 0x4e, 0x30, 0x0d, 
  	                                    0xdb, 0xcc, 0x58, 0x09, 0x09, 0xd4, 0x58, 0x47, 0xc3, 0xd9};
  NAEEM_test_assert_equal_byte_arrays(expected_hash_value, 20, hash, 20, "SHA1 value is incorrect.");
  free(hash);
...
NAEEM_test_SHA256() {
  ...
  NAEEM_result result = NAEEM_crypto_SHA256(message, strlen((const char*)message), hash);
  if (result != NAEEM_RESULT_SUCCESS) {
  ...
  NAEEM_byte expected_hash_value[32] = {0x3a, 0x71, 0x05, 0x4e, 0x51, 0xf0, 0xbc, 0xf3, 0x51, 0x79, 0x2d, 0x77, 0x7e, 0xe4, 0x35, 0xc9, 
  	                                    0xba, 0x5e, 0x18, 0x43, 0xfc, 0x19, 0x84, 0xad, 0x93, 0x74, 0x22, 0x9c, 0x7d, 0x2e, 0xac, 0xb1};
  NAEEM_test_assert_equal_byte_arrays(expected_hash_value, 32, hash, 32, "SHA256 value is incorrect.");
  free(hash);
...
NAEEM_test_DES() {
  ...
  NAEEM_util_print_array("DES DATA", data, 9);
  NAEEM_uint32 cipher_length = 0;
  NAEEM_data cipher = (NAEEM_data)malloc(16 * sizeof(NAEEM_byte));
  NAEEM_DES_key key = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07};
  NAEEM_result result = NAEEM_crypto_DES_cbc_encrypt(data, 9, cipher, &cipher_length, key);
  if (result != NAEEM_RESULT_SUCCESS) {
    printf("> FAILURE: DES ecnryption is not working!\n");
    exit(1);
  }
  printf("DES cipher length: %d\n", cipher_length);
  NAEEM_test_assert_equal_uint32(16, cipher_length, "DES cipher length is wrong.");
  NAEEM_util_print_array("DES CIPHER", cipher, cipher_length);
  NAEEM_data plain = (NAEEM_data)malloc(16 * sizeof(NAEEM_byte));
  NAEEM_uint32 plain_length = 0;
  result = NAEEM_crypto_DES_cbc_decrypt(cipher, cipher_length, plain, &plain_length, key);
  if (result != NAEEM_RESULT_SUCCESS) {
    printf("> FAILURE: DES decryption is not working!\n");
    exit(1);
  }
  printf("Plain length: %d\n", plain_length);
  NAEEM_test_assert_equal_uint32(16, plain_length, "DES plain length is wrong.");
  NAEEM_util_print_array("DES PLAIN", plain, plain_length);
  NAEEM_test_assert_equal_byte_arrays(data, 9, plain, 9, "DES is working.");
  free(cipher);
  free(plain);
...
NAEEM_test_3DES() {
  NAEEM_byte data[9] = {0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99};
  NAEEM_util_print_array("3DES DATA", data, 9);
  NAEEM_uint32 cipher_length = 0;
  NAEEM_data cipher = (NAEEM_data)malloc(16 * sizeof(NAEEM_byte));
  ...
  NAEEM_result result = NAEEM_crypto_3DES_cbc_encrypt(data, 9, cipher, &cipher_length, key1, key2, key3);
  if (result != NAEEM_RESULT_SUCCESS) {
    printf("> FAILURE: 3DES ecnryption is not working!\n");
    exit(1);
  }
  printf("3DES cipher length: %d\n", cipher_length);
  NAEEM_test_assert_equal_uint32(16, cipher_length, "3DES cipher length is wrong.");
  NAEEM_util_print_array("3DES CIPHER", cipher, cipher_length);
  NAEEM_data plain = (NAEEM_data)malloc(16 * sizeof(NAEEM_byte));
  NAEEM_uint32 plain_length = 0;
  result = NAEEM_crypto_3DES_cbc_decrypt(cipher, cipher_length, plain, &plain_length, key1, key2, key3);
  if (result != NAEEM_RESULT_SUCCESS) {
    printf("> FAILURE: 3DES decryption is not working!\n");
    exit(1);
  }
  printf("Plain length: %d\n", plain_length);
  NAEEM_test_assert_equal_uint32(16, plain_length, "3DES plain length is wrong.");
  NAEEM_util_print_array("3DES PLAIN", plain, plain_length);
  NAEEM_test_assert_equal_byte_arrays(data, 9, plain, 9, "3DES is working.");
  free(cipher);
  free(plain);
