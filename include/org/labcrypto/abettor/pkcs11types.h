...
#define MAINTENANCE_ROLE
#define MSD_MANAGER_ROLE       //added by sebt to support MSD manager login
...
/* user types */
#define CKU_PUBLIC						-1		/* no user is logged in! */

#define CKU_SO                          0       /* Security Officer */

#define CKU_USER                        1       /* Normal user      */

#ifdef MAINTENANCE_ROLE
#define CKU_MAINT						2		/* Maintanance      */
#endif
...
#ifdef MAINTENANCE_ROLE
#define CKS_RW_MAINT_FUNCTIONS          5
#endif
...
/**/
/*---------------------------------------------------------------------*
   <<< ResVals - SSW Functions return codes >>>
 *---------------------------------------------------------------------*/
...
#define  CKR_VENDOR_DEFINED0                    0x82000000
#define  CKR_VENDOR_DEFINED1                    0x82100001
#define  CKR_VENDOR_DEFINED2                    0x82200002
#define  CKR_VENDOR_DEFINED3                    0x82300003
#define  CKR_VENDOR_DEFINED4                    0x82400004
#define  CKR_VENDOR_DEFINED5                    0x82500005
#define  CKR_VENDOR_DEFINED6                    0x82600006
#define  CKR_VENDOR_DEFINED7                    0x82700007
#define  CKR_VENDOR_DEFINED8                    0x82800008
#define  CKR_VENDOR_DEFINED9                    0x82900009
#define  CKR_VENDOR_DEFINED10                   0x82a0000a
#define  CKR_SERVICE_TIMEOUT					0x8000000b
#define  CKR_BUFFER_SIZE						0x8000000c

#define  CKR_OK_AND_RETURN						0x80000fff
#define  CKR_VENDOR_DEFINED_BAD_FIRMWARE        0x80000bad

//#define  CKR_VENDOR_TOKEN                       0x90000000
...
//ToDo: CK_ENTRY is public interface, should be testesd in all platforms/compilers
#if defined _WIN32 || defined __CYGWIN__
    #ifdef __GNUC__
      #define CK_ENTRY __attribute__ ((dllexport))
    #else
      #define CK_ENTRY __declspec(dllexport) // Note: actually gcc seems to also supports this syntax.
    #endif
#else
  #if __GNUC__ >= 4
...
//added by sebt, set picode limitations for USER and SO
typedef CK_RV CK_ENTRY  (CK_PTR CK_C_SetPinConstraints) (CK_SLOT_ID          SlotId,
														 CK_SESSION_HANDLE   HSession,
														 CK_ULONG			 compLevel,
														 CK_ULONG			 retryNumber,
														 CK_ULONG			  minLen,
														 CK_ULONG			  maxLen);

typedef CK_RV CK_ENTRY (CK_PTR CK_C_GetPinConstraints)
														(CK_SLOT_ID              SlotId,
...
typedef CK_RV CK_ENTRY  (CK_PTR  CK_C_WaitForSlotEvent) (CK_FLAGS flags,
                                                         ...
                                                         CK_VOID_PTR pReserved);
//added by sebt called when test is successfully passed
typedef CK_RV CK_ENTRY  (CK_PTR CK_testPassed) (CK_SLOT_ID          SlotId);

#ifdef MSD_MANAGER_ROLE
typedef CK_RV CK_ENTRY (CK_PTR CK_C_SetFlashState) (
							CK_SESSION_HANDLE       HSession,
								CK_ULONG				state
															);
#endif

typedef CK_RV CK_ENTRY  (CK_PTR CK_C_SetFIPSMode)  (CK_SLOT_ID              SlotId,
													CK_SESSION_HANDLE       HSession,
														CK_ULONG                mode);

#ifdef MAINTENANCE_ROLE
typedef CK_RV CK_ENTRY  (CK_PTR  CK_C_Maint_Recover_Firmware_Lock) (CK_SLOT_ID              SlotId,
															CK_SESSION_HANDLE       HSession,
															CK_ULONG                password,
	                                                        CK_BYTE_PTR		        pData,
															CK_ULONG_PTR			pDataLen);
typedef CK_RV CK_ENTRY  (CK_PTR  CK_C_Maint_Recover_Selftest_Fail) (CK_SLOT_ID              SlotId,
															CK_SESSION_HANDLE       HSession,
															CK_ULONG                password,
	                                                        CK_BYTE_PTR		        pData,
															CK_ULONG_PTR			pDataLen);
typedef CK_RV CK_ENTRY  (CK_PTR  CK_C_Maint_Unlock_SOPIN)  (CK_SLOT_ID              SlotId,
															CK_SESSION_HANDLE       HSession,
															CK_ULONG                password);
/*typedef CK_RV CK_ENTRY  (CK_PTR  CK_C_Maint_Change_PIN)  (CK_SLOT_ID              SlotId,
																CK_ULONG                password,
																CK_ULONG                new_password);*/
#endif

typedef CK_RV CK_ENTRY (CK_PTR CK_C_IOTest) (
									CK_SLOT_ID SlotId,
									CK_BYTE *password,
									CK_BYTE *data,
									int offset,
...
typedef CK_RV CK_ENTRY (CK_PTR CK_C_WaitForObjectEvent)
(
        CK_FLAGS                flags,
        CK_SLOT_ID_PTR          pSlot,
        CK_ULONG				objFlags,
		CK_ULONG_PTR            objAction
);
...
struct CK_FUNCTION_LIST
{
		...
		CK_C_SetPinConstraints			C_SetPinConstraints;//added by sebt, set pin limts, fw > 3.87
		CK_C_GetPinConstraints			C_GetPinConstraints;//added by sebt, get pin limts, fw > 3.91
		CK_testPassed			        testPassed;//added by sebt, successful test pass fw > 3.99
#ifdef MSD_MANAGER_ROLE
		CK_C_SetFlashState				C_SetFlashState;
#endif
	    CK_C_SetFIPSMode				C_SetFIPSMode;
#ifdef MAINTENANCE_ROLE
		CK_C_Maint_Recover_Firmware_Lock	C_Maint_Recover_Firmware_Lock;
		CK_C_Maint_Recover_Selftest_Fail	C_Maint_Recover_Selftest_Fail;
		CK_C_Maint_Unlock_SOPIN				C_Maint_Unlock_SOPIN;
		//CK_C_Maint_Change_PIN				C_Maint_Change_PIN;
#endif

	   CK_C_IOTest						 C_IOTest;
	   CK_C_WaitForObjectEvent           C_WaitForObjectEvent;
};;
...
#ifdef MAINTENANCE_ROLE
CK_RV CK_ENTRY C_Maint_Recover_Firmware_Lock    (CK_SLOT_ID, CK_SESSION_HANDLE, CK_ULONG, CK_BYTE_PTR, CK_ULONG_PTR);
CK_RV CK_ENTRY C_Maint_Recover_Selftest_Fail    (CK_SLOT_ID, CK_SESSION_HANDLE, CK_ULONG, CK_BYTE_PTR, CK_ULONG_PTR);
CK_RV CK_ENTRY C_Maint_Unlock_SOPIN				(CK_SLOT_ID, CK_SESSION_HANDLE, CK_ULONG);
//CK_RV CK_ENTRY C_Maint_Change_PIN				(CK_SLOT_ID, CK_SESSION_HANDLE, CK_ULONG, CK_ULONG);
#endif
CK_RV CK_ENTRY C_SetFIPSMode			(CK_SLOT_ID, CK_SESSION_HANDLE, CK_ULONG);
CK_RV CK_ENTRY C_SetPinConstraints(CK_SLOT_ID SlotId, CK_SESSION_HANDLE HSession, CK_ULONG compLevel, CK_ULONG retryNumber, CK_ULONG minLen, CK_ULONG	maxLen); // added by sebt, set pin limitations, fw > 3.87
CK_RV CK_ENTRY C_GetPinConstraints(CK_SLOT_ID SlotId, CK_USER_TYPE userType, CK_ULONG *compLevel, CK_ULONG *retryNumber, CK_ULONG *minLen, CK_ULONG	*maxLen);  // added by sebt, get pin const
#ifdef WIN32
